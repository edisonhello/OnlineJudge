...
int32_t main() {
    ...
    cin >> n >> m;
    vector<int> v, vec;
    for (int i = 0; i < n; ++i) cin >> f[i].first.first >> f[i].first.second, f[i].second = i, v.push_back(f[i].first.first);
    f[n].first.first = inf;
    for (int i = 0; i < m; ++i) cin >> mos[i].first >> mos[i].second, v.push_back(mos[i].first);
