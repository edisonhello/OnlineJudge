// Big Integer adopted from Sunmoon
#include <bits/stdc++.h>
#include "lib1417.h"
...
class bigN{
	...
	inline friend std::pair<bigN,bigN>divmod(const bigN &a,const bigN &b){
		...
		for(int i=x.s.size()-1;i>=0;i--){
			...
			int d=((int)SM_BASE*s1+s2)/y.s.back();
			r-=y*d;
...
int main() {
    ...
    int T; cin >> T; while (T--) {
        ...
        nth_element(vec.begin(), vec.begin() + s - 1, vec.end());
        nth_element(vec.begin() + s - 1, vec.begin() + t, vec.end());
        bigN num = 0;
        for (int i = s - 1; i < t; ++i) num += vec[i];
